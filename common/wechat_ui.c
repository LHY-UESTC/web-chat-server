...
void show_msg(struct wechat_msg *msg) {
    ...
    if (msg->type & WECHAT_SYS) {
        ...
        strcpy(msg->name, "SysInfo");
        wattron(sub_msg_win, COLOR_PAIR(1));
    } else {
        wattron(sub_msg_win, COLOR_PAIR(4));
    }
    w_gotoxy_puts(sub_msg_win, 9, msgcnt, msg->name);
    //消除颜色
    wattron(sub_msg_win, COLOR_PAIR(7));
    w_gotoxy_puts(sub_msg_win, 10 + strlen(msg->name), msgcnt, msg->msg);
    int tmp = (strlen(msg->msg) + 11 + strlen(msg->name)) / (MSG_WIDTH - 2) + 1;
    msgcnt += tmp;
    wmove(sub_input_win, 2, 1);
    wrefresh(sub_input_win);
    if (flag) strcpy(msg->name, pre_name);
}
